...
pub fn parse(code: String, allow_wrong_line_comments: bool) -> Result<JsValue, JsError> {
    ...
    let result = match swc_css_parser::parse_file(
        ...
        Ok(stylesheet) => ParserResult {
            ast: Some(stylesheet),
            errors: errors.into_iter().map(convert_error).collect(),
        },
        Err(error) => ParserResult {
            ast: None,
            errors: vec![convert_error(error)],
        },
    };
...
fn convert_error(error: swc_css_parser::error::Error) -> Error {
